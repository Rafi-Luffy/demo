...
// @access  Private
router.get('/', auth_js_1.protect, async (req, res, next) => {
    try {
        const { page = 1, limit = 10, campaignId, status, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        ...
        if (req.user?.role === 'donor') {
            ...
            const campaigns = await Campaign_js_1.Campaign.find({ creator: req.user.id }).select('_id');
        ...
        const donations = await Donation_js_1.Donation.find(query)
        ...
        const total = await Donation_js_1.Donation.countDocuments(query);
        res.status(200).json({
            success: true,
...
router.get('/:id', auth_js_1.protect, async (req, res, next) => {
    try {
        const donation = await Donation_js_1.Donation.findById(req.params.id)
            .populate('donor', 'name email profile')
            .populate('campaign', 'title description creator');
        if (!donation) {
            return next(new index_js_1.AppError('Donation not found', 404));
        }
        // Check if user can view this donation
        if (req.user?.role !== 'admin' &&
            req.user?.id !== donation.donor._id.toString() &&
            req.user?.id !== donation.campaign.creator.toString()) {
            return next(new index_js_1.AppError('Not authorized to view this donation', 401));
        }
        res.status(200).json({
            success: true,
            data: donation
...
// @access  Private
router.post('/', auth_js_1.protect, async (req, res, next) => {
    try {
        ...
        if (campaign.status !== 'active') {
            return next(new index_js_1.AppError('Campaign is not accepting donations', 400));
        }
        // Check if campaign has ended
        if (campaign.endDate && new Date() > campaign.endDate) {
            return next(new index_js_1.AppError('Campaign has ended', 400));
        }
        // Check for duplicate transaction hash
        if (transactionHash) {
            const existingDonation = await Donation_js_1.Donation.findOne({ transactionHash });
            if (existingDonation) {
                return next(new index_js_1.AppError('Transaction already processed', 400));
            }
        }
        // Create donation
        ...
        if (donation.status === 'confirmed') {
            await Campaign_js_1.Campaign.findByIdAndUpdate(campaignId, {
                $inc: {
                    raisedAmount: amount,
                    donorCount: 1
                }
        ...
        const populatedDonation = await Donation_js_1.Donation.findById(donation._id)
            .populate('donor', 'name profile.avatar')
...
router.put('/:id/status', auth_js_1.protect, async (req, res, next) => {
    try {
        ...
        if (gasUsed)
            donation.gasUsed = gasUsed;
        if (gasFee)
            donation.gasFee = gasFee;
        await donation.save();
        // Update campaign totals if status changed to confirmed
        if (oldStatus !== 'confirmed' && status === 'confirmed') {
            await Campaign_js_1.Campaign.findByIdAndUpdate(donation.campaign, {
                $inc: {
...
// @access  Private
router.get('/analytics/stats', auth_js_1.protect, async (req, res, next) => {
    try {
        ...
        const analytics = await Donation_js_1.Donation.aggregate([
            { $match: matchQuery },
            {
                $group: {
                    _id: null,
        ...
        // Get daily breakdown
        const dailyStats = await Donation_js_1.Donation.aggregate([
            { $match: matchQuery },
            {
                $group: {
                    _id: {
...
// @access  Private
router.get('/:id/receipt', auth_js_1.protect, async (req, res, next) => {
    try {
        const donation = await Donation_js_1.Donation.findById(req.params.id)
            .populate('donor', 'name email profile')
        ...
        if (req.user?.id !== donation.donor._id.toString() && req.user?.role !== 'admin') {
            return next(new index_js_1.AppError('Not authorized to access this receipt', 401));
        }

...
const upload = (0, multer_1.default)({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (allowedTypes.includes(file.mimetype)) {
            ...
            cb(new index_js_1.AppError('Invalid file type. Only PDF, DOC, DOCX, images, and text files are allowed.', 400));
        }
    }
});
...
// @access  Private
router.post('/upload', auth_js_1.protect, upload.single('file'), async (req, res, next) => {
    try {
        if (!req.file) {
            return next(new index_js_1.AppError('No file provided', 400));
        }
...
// @access  Private
router.post('/upload-json', auth_js_1.protect, async (req, res, next) => {
    try {
        const { data, metadata } = req.body;
        if (!data) {
            return next(new index_js_1.AppError('No data provided', 400));
        }
...
router.get('/:hash', async (req, res, next) => {
    try {
        ...
        if (!IPFSService.validateHash(hash)) {
            return next(new index_js_1.AppError('Invalid IPFS hash format', 400));
        }
        const file = await IPFSService.getFile(hash);
        if (!file.available) {
            return next(new index_js_1.AppError('File not available on IPFS', 404));
        }
        res.status(200).json({
            success: true,
            data: {
...
router.post('/:hash/pin', auth_js_1.protect, async (req, res, next) => {
    try {
        const { hash } = req.params;
        if (!IPFSService.validateHash(hash)) {
            return next(new index_js_1.AppError('Invalid IPFS hash format', 400));
...
// @access  Private
router.post('/upload-multiple', auth_js_1.protect, upload.array('files', 10), async (req, res, next) => {
    try {
        const files = req.files;
        if (!files || files.length === 0) {
            return next(new index_js_1.AppError('No files provided', 400));
        }
...
// @access  Private
router.get('/search', auth_js_1.protect, async (req, res, next) => {
    try {
        const { query, category, uploadedBy, limit = 20 } = req.query;
